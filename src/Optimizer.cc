...
void Optimizer::GlobalBundleAdjustemnt(Map* pMap,
                                       ...
                                       const bool bRobust,
                                       const std::vector<unsigned long> &fixedKFs)
{
  // 获取地图中的所有关键帧
  ...
  // 调用GBA
  BundleAdjustment(vpKFs, vpMP, nIterations, pbStopFlag, nLoopKF, bRobust, fixedKFs);
}
                                 ...
                                 const std::vector<unsigned long> &fixedKFs)
{
  ...
  for(auto pKF : vpKFs)
  {
    ...
    vSE3->setId(pKF->mnId); // 顶点的id就是关键帧在所有关键帧中的id
    if (std::find(fixedKFs.begin(), fixedKFs.end(), pKF->mnId) != fixedKFs.end())
      vSE3->setFixed(true);
  ...
  for(size_t i=0; i<vpMP.size(); i++)
  {
    ...
    // 获取优化之后的地图点的位置
    g2o::VertexSBAPointXYZ* vPoint = dynamic_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnId+maxKFid+1));
 ...
 */
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, bool fixedPose)
{
  ...
  for(auto & lLocalMapPoint : lLocalMapPoints)
  {
    ...
    for(auto & observation : observations)
    {
      ...
      // pKFi->mnLocalBAForKF!=pKF->mnId 表示不属于局部关键帧，
  ...
  for(auto pKFi : lLocalKeyFrames)
  {
    ...
    // 如果是初始关键帧，要锁住位姿不优化
    vSE3->setFixed(pKFi->mnId==0 || fixedPose);
    optimizer.addVertex(vSE3);
                                       ...
                                       const bool &bFixScale)
{
  ...
  for(auto pKFi : vpKFs)
  {
    ...
    g2o::VertexSim3Expmap* VSim3 = dynamic_cast<g2o::VertexSim3Expmap*>(optimizer.vertex(nIDi));
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
  ...
  g2o::VertexSim3Expmap* vSim3_recov = dynamic_cast<g2o::VertexSim3Expmap*>(optimizer.vertex(0));
