...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint*> &vpMapPoints, const float th)
{
  ...
  for(auto pMP : vpMapPoints)
  {
    ...
    const int &nPredictedLevel = pMP->mnTrackedScaleLevel;

    // The size of the window will depend on the viewing direction
    // Step 2 设定搜索搜索窗口的大小。取决于视角, 若当前视角和平均视角夹角较小时, r取一个较小的值
    float r = RadiusByViewingCos(pMP->mTrackedViewCosine);
    ...
    const vector<size_t> vIndices =
      F.GetFeaturesInArea(pMP->mTrackedProjX, pMP->mTrackedProjY,      // 该地图点投影到一帧上的坐标
 ...
 * @param[in & out] vpMatched   当前KF的已经匹配的地图点
 * @param[in] th                搜索范围
 ...
 */
int ORBmatcher::SearchForInitialization(const Frame &F1, const Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
  int nmatches=0;
 ...
 * @param  vpMatches12        pKF2中与pKF1匹配的MapPoint，vpMatches12[i]表示匹配的地图点（来自pKF2），null表示没有匹配，i表示匹配的pKF1 特征点索引
 * @return                    成功匹配的数量
 */
